...
1. **例外体系** — 正準は `Sources/Error/` 配下の型（いずれも LocalizedError）。最外殻変換点は `Sources/MCP/main.swift` の CallTool ハンドラ（`errorText()`）。外部境界（ファイル I/O・JSON decode 等）のエラーはこの体系へ翻訳してから投げる。
...
6. **レイヤー定義の正準の所在** — `.swiftlint.yml` のコメント（`# Architecture:` で始まるレイヤー順コメント）と custom_rules。
