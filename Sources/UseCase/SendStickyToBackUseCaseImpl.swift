final class SendStickyToBackUseCaseImpl: AsyncUseCase, Sendable {
    ...
    func execute(_ request: SendStickyToBackRequest) async throws -> BoardMutationResponse {
        let newState = try await stickyService.sendToBack(id: request.stickyID)
